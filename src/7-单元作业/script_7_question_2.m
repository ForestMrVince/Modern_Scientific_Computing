...
% 并且打到靶子上的目标值相差不大，如果需要更精确的解，请自行提高ode求解的精度要求。
% 对于解的唯一性，本答案也进行了尝试，虽然单独去取值并不能证明解的唯一性。对于第一问，初始点的y2取到了正负几千，依然没有找到当x=5时y1=3的点。对于第二问，图像随m变化很大，但最起码m在区间(-5， -4)间还存在一个满足要求的解，可见第二问解不唯一，只需不断循环就能找到更多的解，但找不找得到所有的解，以及解有多少个不得而知，所以以下代码只是搜索代码，只找到了其中的一个解。
clear; clc;
% 函数 y = f(x)
% y1 = y;
% y2 = diff(y1) = diff(y);
...
m_search_step = 0.0001;   % m_search_factor 的搜索步长, 为正时对应搜索因子增大
